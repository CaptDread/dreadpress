...
gulp.task('sass-compile', (done) => {
	return gulp
		...
		.pipe(gulp.dest((file) => file.base.replace(path.sep + 'src', path.sep + 'sass').replace(path.sep + '', path.sep + 'css'))) // put the css files here.
		...
		.pipe(
			gulpFn(function (file) {
				if (file.path.indexOf('.css.map') === -1) {
					...
					console.log(file.path.replace(path.sep + 'src', path.sep + 'sass').replace(path.sep + '', path.sep + 'css'));
				}
			})
		);
});
...
gulp.task('start-browsersync', function (done) {
	browserSync.init({
        ...
        proxy: 'http://localhost:81/wp-admin'
	});
	done();
});
​
